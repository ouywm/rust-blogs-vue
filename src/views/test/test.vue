    ...
    <art-table :data="tableData">
      <template #default>
        ...
        <el-table-column fixed="right" label="操作" width="180">
          <template #default="scope">
            <button-table type="add" v-auth="'add'" @click="showModel('menu', null,false)" />
...
<script setup lang="ts">
  ...
  const getMenuList = async () => {
    try {
      ...
      ElMessage.error('获取菜单列表失败', error)
    }
  }
  ...
  const submitForm = async () => {
    ...
    await formRef.value.validate(async (valid) => {
      if (!valid) return
      try {
        const { menu_id } = currentRow.value || {}
        const isMenuType = labelPosition.value === 'menu'
        const baseParams = {
          title: isMenuType ? form.name : form.authName,
          icon: form.icon,
          is_enable: form.isEnable ? 1 : 0,
          keep_alive: form.keepAlive ? 1 : 0,
          is_hide: form.isHidden ? 1 : 0,
          link: form.link,
          is_iframe: form.is_iframe ? 1 : 0,
          ...
          parent_id: lockMenuType.value ? 0 : currentRow.value?.menu_id || 0
        }
        ...
        // 处理按钮权限校验
        if (labelPosition.value === 'button' && !menu_id) {
          return ElMessage.error('按钮权限必须指定父级菜单')
        }
